...
{
	// Mapping texture samplers
	GLuint textureUnit = GL_TEXTURE0;
	for (uint i = 0; i < MAX_TEXTURES; i++)
	{
		this->m_arraySamplerIdToUnit[i] = textureUnit + i;
	}
}
...
GLuint cBasicTextureManager::GetUnitFromSamplerId(uint samplerId)
{
	if (samplerId >= MAX_TEXTURES)
	{
		return 0;
	}
	return this->m_arraySamplerIdToUnit[samplerId];
}

std::string cBasicTextureManager::GetSamplerName(uint samplerId)
{
	return "texture" + std::to_string(samplerId);
}
